...
print('Connecting to the PostgreSQL successful')
class Proxy:
    ...
    def inject_code(self, pkt):
        ...
        if http_packet.haslayer(HTTPRequest):
            print("HTTP Request from "+http_packet[IP].src+" to "+http_packet[IP].dst+ " received")
            self.check_path(pkt)
...
try:
    ...
    print("Proxy listening on "+ config.proxy_IP + " reverse proxying for " + config.server_IP)
